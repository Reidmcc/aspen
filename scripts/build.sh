...
if [[ ($# -gt 1 || `pwd | rev | cut -d'/' -f1 | rev` != "aspen") ]]
then
    echo "need to invoke from the root 'aspen' directory"
    exit 1
fi
...
LDFLAGS="-X github.com/Reidmcc/aspen/cmd.version=$VERSION_STRING -X github.com/Reidmcc/aspen/cmd.gitBranch=$GIT_BRANCH -X github.com/Reidmcc/aspen/cmd.gitHash=$GIT_HASH -X github.com/Reidmcc/aspen/cmd.buildDate=$DATE"
...
do
    ...
    # archive
    ARCHIVE_FILENAME=aspen-$VERSION-$GOOS-$GOARCH$GOARM.tar
    cd $ARCHIVE_DIR
...
echo -n "cleaning up aspen folder: $ARCHIVE_DIR_SOURCE/ ... "
cd $ARCHIVE_DIR
